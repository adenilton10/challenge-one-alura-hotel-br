...
public class ReservasView extends JFrame {
		...
		btnProximo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {
					...
					Reserva reserva = salvarReserva(); // salvando reserva no banco de dados
					registro.setIdReserva(reserva);  // exibindo o idReserva no views de RegistroHospede
	...
	public Reserva salvarReserva() {
		...
		Reserva reserva = new Reserva(java.sql.Date.valueOf(dataEntrada), java.sql.Date.valueOf(datasaida),
				txtValor.getText(), txtFormaPagamento.getSelectedItem().toString());
		reservaController.salvar(reserva);
		return reserva;
	}

	public void setarValorReserva() {// calcula e exibe o valor da reserva de acordo com as datas selecionadas
		if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null) {
			...
			LocalDate lDdataEntrada = LocalDate.parse(dataEntrada, formato);
			LocalDate lDdataSaida = LocalDate.parse(datasaida, formato);
			...
			int resultado = (diasDoAnoDataSaida - diasDoAnoDataEntrada) * 20;

			txtValor.setText("R$" + resultado);

			if (diasDoAnoDataSaida < diasDoAnoDataEntrada) {
				JOptionPane.showMessageDialog(contentPane, "Data de saida não pode ser menor que a da entrada.");
			}
